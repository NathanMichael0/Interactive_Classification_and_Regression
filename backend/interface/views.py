...
def reg(request):
    ...
    if (userChoice == "linreg"):
        reg_clf = eval(
            "LinearRegression("+parameterString[0:len(parameterString) - 1]+")")
        ...
        reg_clf = eval(
            "LogisticRegression("+parameterString[0:len(parameterString) - 1]+")")
        ...
        reg_clf = eval(
            "Ridge("+parameterString[0:len(parameterString) - 1]+")")
...
def clf(request):
    ...
    if (userChoice == "logreg"):
        ...
        logreg_fpr, logreg_tpr, logreg_threshold = metrics.roc_curve(
            y_test, logreg_preds)
        ...
        reg_clf = eval(
            "LinearSVC("+parameterString[0:len(parameterString) - 1]+")")
        reg_clf.fit(X_train, y_train)
        i = 0
        for name in (reg_clf.feature_names_in_):
            featureImp.append({"name": name, "coef": (reg_clf.coef_)[0][i]})
        ...
        reg_clf = eval("RandomForestClassifier(" +
                       parameterString[0:len(parameterString) - 1]+")")
        reg_clf.fit(X_train, y_train)
...
# 10-fold cross validation with scoring='roc_auc'
#  validation done on the whole X
    cv_score = cross_val_score(
        reg_clf, X, np.ravel(y), cv=10, scoring='roc_auc')
